...
"%PSEXE%" -NoProfile -Command "(Get-Date).AddDays(-30).ToString('yyyy-MM-dd')" > "%TEMP%\dc_compras_1.txt"
set /p DATA_CORTE=<"%TEMP%\dc_compras_1.txt"

echo. >> %logfile% 2>&1
echo === DELTA INICIO: %DATE% %TIME% (DATA_CORTE=%DATA_CORTE%) === >> %logfile% 2>&1

"%kitchen%" /file:"%currentdir%job_compras_loja_1_economico.kjb" "/param:LOJA_ID=1" "/param:DATA_CORTE=%DATA_CORTE%" /level:Basic >> %logfile% 2>&1
